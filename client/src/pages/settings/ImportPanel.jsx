...
export default function ImportPanel() {
  ...
  const onPick = async (e) => {
    ...
    if (file.size > MAX_BYTES) {
      setError(t("import.err.too_big", { default: "Fichier trop volumineux (max 8 Mo)." }));
      return;
    }
    try {
      ...
      setError(t("import.err.parse", { default: "Ce fichier n'est pas un JSON valide." }));
    }
  };
  ...
  return (
    <div className="mt-8 border-t border-[var(--border-subtle)] pt-6">
      <p className="micro">{t("import.kicker", { default: "復 · RESTAURATION" })}</p>
      <h3 className="display text-lg mt-1 text-[var(--color-ivoire)]">
        {t("import.title", { default: "Réimporter une sauvegarde" })}
      </h3>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <Button as="label" htmlFor="import-file" variant="subtle" iconStart={<Upload size={15} />}>
          {t("import.pick", { default: "Choisir un fichier…" })}
        </Button>
        {fileName ? (
        ...
        <>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <Button onClick={() => run(true)} disabled={busy} variant="subtle">
              {t("import.preview", { default: "Aperçu (n'écrit rien)" })}
            </Button>
            {plan ? (
              <Button onClick={() => run(false)} disabled={busy}>
                {t("import.apply", { default: "Importer maintenant" })}
              </Button>
            ) : null}
          </div>
        </>
      ...
      {plan ? (
        <dl className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
          <Stat label={t("import.stat.collection", { default: "Pièces lues" })} v={plan.collection_rows} />
          <Stat label={t("import.stat.wishlist", { default: "Souhaits lus" })} v={plan.wishlist_rows} />
          <Stat label={t("import.stat.matched", { default: "Appariées" })} v={plan.matched_figures} />
          <Stat label={t("import.stat.new", { default: "À créer" })} v={plan.new_figures} />
          <Stat label={t("import.stat.present", { default: "Déjà présentes" })} v={plan.already_present} />
        </dl>
      ) : null}
