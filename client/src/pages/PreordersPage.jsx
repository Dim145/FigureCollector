...
/**
 * "S'abonner au calendrier" — a quiet, collapsible affordance under the hero
 * that hands the user their private iCal feed URL (subscribable in Google /
 * Apple / Outlook). The token is minted lazily (only when the panel opens) and
 * can be rotated to revoke a leaked link. Direction-A: noir card, gold hairline.
 */
function CalendarSubscribe({ t }) {
  ...
  return (
    <div className="mt-7 flex flex-col items-center">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-[var(--color-ivoire-soft)] hover:text-[var(--color-or)] transition-colors"
      >
        <span aria-hidden className="ja not-italic text-[var(--color-or)] text-sm">暦</span>
        {t("preorders.ical.cta", { default: "S'abonner au calendrier" })}
        <span aria-hidden className="text-[var(--color-ivoire-soft)]/40">{open ? "▴" : "▾"}</span>
      </button>
            ...
            <>
              <div className="flex items-stretch gap-2">
                ...
                <button
                  type="button"
                  onClick={copy}
                  className="shrink-0 px-3 py-2 border border-[var(--color-or)]/30 text-[11px] uppercase tracking-[0.18em] text-[var(--color-or)] hover:bg-[var(--color-or)]/10 transition-colors"
                >
                  {copied
              ...
              <div className="mt-4 flex items-center justify-between gap-3">
                <a
                  href={webcalUrl}
                  className="inline-flex items-center gap-2 text-[12px] uppercase tracking-[0.18em] text-[var(--color-jade)] hover:text-[var(--color-or)] transition-colors"
                >
                ...
                <button
                  ...
                  className="text-[11px] uppercase tracking-[0.18em] text-[var(--color-ivoire-soft)]/55 hover:text-[var(--color-laque-bright)] transition-colors disabled:opacity-50"
                >
                  {confirmRotate
